...
<main>
        ...
        @method('PATCH')
            <div class="change-form__content">
                <div class="change-form__content-image">
                    ...
                    <input class="change-form__content-image-item" type="file" accept="images/*" name="image" onchange="previewImage(event); showFileName(event);">
                    <span id="fileName">{{ old('image') ? Storage::url(old('image')) : '' }}</span>
                    @error('image')
  ...
  if (input.files && input.files.length > 0) {
    ...
    // もしoldの画像パスがあってもこの欄はファイル名表示に使えない
    // なので空にするか、保存された画像の名前を表示させたい場合は別途処理
